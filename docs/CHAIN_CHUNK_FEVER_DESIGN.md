...
Memory Dungeon has a deep rules layer and almost no *payoff*. A correct match turns two tiles grey. The streak is a number in the HUD. Nothing on the board ever happens *because* you were good. Bubble shooters and Peggle are addictive for one reason above all others: **one small, skilled input produces a large, visible, escalating consequence on the board** — and the consequence is bigger the better you have been playing. This document takes that loop and translates it honestly into a memory game: every tile carries a **suit** you can see on its back; **every match pops** — the same-suit cards touching the two tiles you matched break away with them, live, the moment the second tile turns; a **chain** of correct matches is what lets the pop **ripple** — each card that breaks seeds the next wave, and from Sharp the wave crosses into the clump it was touching, wave after wave; and a long enough chain tips into **Fever**, the celebration beat. The dungeon is not beside this loop; it is inside it — pops are how you hit enemies, traps are what stops a pop, the exit is the last peg. Section 8 is the second pass, made after the first ten generations shipped: the brief said *while we're playing, not at the end*, and the first pass had gated the pop behind the chain. Section 10 is the third: the rule from section 8 was correct and the generated boards could not use it, so on floors 1 to 6 of a real run nothing popped at all.
...
| Momentum | Tier name | What it unlocks (as of §10) |
|---|---|---|
| 0–2 | — | **the pop**: two steps into the clump touching the match, pairs whose both halves are inside it |
| 3+ | **Clean** | **depth**: the wave walks four steps along the clump rather than two — the same pop, twice as far |
...
| max(7, half the floor's pairs) | **Fever** | Sharp, but three bridged clumps rather than one, and the wave walks diagonals + celebration (the shard burst went with the shard in Gen 184) |
...
- **Fever**: Sharp's reaction, bridging into **three** touching clumps rather than one, and walking **diagonally**, so a clump that meets another only at a corner is one clump. Peggle's fever lights every peg left; here the fire jumps every contact it can find. (Until Gen 197 this was a *halo* — every bordering tile whatever its suit, taken at a distance the player could not read. See `BREAK_TOUCHES_ONLY.md`.)
...
What the player sees: they match a pair inside a big Ember chunk, and the Ember chunk *goes* — a wave of shatters spreading outward from the match along the clump, and at Sharp and above jumping into the clump next door where the two were touching, the pitch climbing with each one. Nothing pops that the wave did not reach by contact. That is the drop. That is the whole reason to build a chain.
...
At the Fever rung the break is a **Fever break** — the clump and the three clumps it was touching go — the board dims and pulses gold, time stretches for a beat (a 450 ms slow-in on the shatter wave, respecting `reduceMotion`), a distinct sting plays, and the run line names it. Every cascaded pair in a Fever break drops a shard, not every second one.
...
| **124** | Style shots, done: the turn event carries the chunk's shape (`chunkPartnerSpanMax`, `chunkHaloPairs`, `chunkTreasuresSpilled`, `chunkSuitCleared`), read off the two boards by the fact stamper; the run line names it in one line after the break — "Long clump", "Bridge", "Treasure spill ×n", "Clean sweep". The pitch ladder already steps once per pair with a cap of twelve. | Fact tests, copy tests, the announcement test; Codex v21. |
...
| **130** | Relics that touch the cascade, done: Tuning Fork (a lone match reaches its partners; from Gen 168 a Sharp break also feeds the chain its whole pop), Magpie's Ledger (spilled treasure pays double gold, matched treasure unchanged) and Suit Lens (one suit fewer than the floor would deal, down to two, for bigger clumps) — standing rules in the pool with draft rows, Codex entries and draft-card copy, read by `resolveChunkBreak` and `dealBoardSuits`. The sim gained a loadout axis (`--relics`): each relic alone sits inside the bare bands; all three together lift a 25%-miss player's Fever share to 0.23, held to a stated relic band of 0.3. | Rule tests per relic, the pool alignment test, the sim loadout test; Codex v26. |
| **131** | Daily and shared play carry the chain, done: the run summary and history keep the longest chain (momentum, not the score streak) and the floors that reached Sharp and Fever, and the persisted summary normaliser now keeps them (it had been dropping Gen 129's `biggestChunk` on reload). The run share line and the daily share string say `best chain ×N` and `Fever on M floors`; the quest ladder gains Chain Rhythm (a Sharp chain on three cleared floors, read from a new `playerStats.sharpFloors` counter with a migration policy, folded in once per clear and never from a shared table). Pass-and-play seats credit the pairs a chunk took on their turn, keep a best chain, and the handoff line says "Player 1's chain of 4 ends. Player 2's turn." when a miss ends a chain of two or more; the standings show each seat's best chain. Fit-contract findings on the way: the relic draft's four offers wrapped to two rows at 1024×768 and pushed the reroll, ban and upgrade buttons below the fold (one row at every width now); the floor-clear recap pushed the third door off a phone (non-reward notes give way there); and the "dungeon showcase" fit case had been clicking a main-menu button Gen 111 retired, timing out at every size and reading as load — it is now the run setup sheet's fit case, which found the sheet's nine 44px rows overflowing a phone held sideways (three columns there). *Correction, 2026-09-23:* the chain kept was never momentum - `bestChain*` is fed the streak, which is right for a number shown beside the HUD's streak - but the Sharp and Fever floor records were read off it too, so a floor the HUD called Fever and whose clear paid Fever could be recorded as Clean. They now read `peakChainTierThisFloor`, the highest rung `runChainTier` showed after any match on the floor, and the run's peak rides on the summary as `peakChainTier` for the game-over chain target. |
| **189** | The ladder re-measured, and Gen 186's finding withdrawn. Gen 186 reported Sharp taking 0.32 pairs more than Clean and filed it as a thin middle rung; measured in score - which is what the tier multiplies and what the player is paid - the rungs pay 70 / 226 / 506 / 2036, so every one of them more than doubles the one below (×3.2, ×2.2, ×4.0). The ladder was never thin, the measurement was. `scorePerMatch` and `scoreStep` are now in the pop simulation and banded (`ladderScoreMinStep`, 1.8×), and the HUD shows the rung's **multiplier** rather than Gen 186's pairs-pips, which is Peggle's ladder and the number the player is paid. Recorded with it: the reach experiment that preceded the measurement (reach 1 gives the best pair ladder, hands a sloppy player Fever on a third of floors against a 0.22 band, and breaks all three authored floors) and the finding that Clean's entire value is the partner reach, so Sharp needs different *boards* rather than a different reach. | Score-ladder band in `sim:pop`, rung-value tests against the live simulation, HUD multiplier test. |
| **188** | Phase 3 closed out, with one task handed back to the product. T3.1, T3.2, T3.3 and T3.5 shipped in Gens 185-187: the aim guide reads the real tier and ghosts the next rung, the meter says what a rung is worth, and the break's score is built term by term. **T3.4 (the held-pair marker) is blocked, not skipped**: the game already ships the pin - three hidden tiles, toggled from the dock, changing no rules, with a contract vow capping a run's total - which is the same job minus the one thing that matters, that a pin marks a tile and the hold decision's unit is a pair. The gap cannot be closed by inference, because `RunState` has no seen-set and that absence is the memory game; the claim has to be a two-tile gesture, which is the gesture the pin owns. Two constraints recorded for whoever builds it: the mark must never validate (a mark that told you whether two tiles match would be a free match test, strictly better than flipping), and T3.6's span may only ever be shown on a pair the player has claimed, where it restates their own choice and leaks nothing. Three options and a recommendation are at thesis §30.3(c); E.6's crutch question is recorded as unsettleable by simulation, because a memory aid's effect on human memory is not a parameter the reference player carries. | Docs only; no rule or interface change. |
| **187** | The score built in front of the player, done (thesis T3.5, §40.4). Gen 181 made the break's score multiplicative - a pair's worth, times the pairs, times the tier, times the ripple - and then multiplied the terms out of sight, so the player met only the product. `score-terms-rules.ts` returns those terms with a running total, rebuilt from the same functions `chunkBreakScore` uses so the line cannot promise a product the run did not award, and the floater reveals them one beat at a time: `4 pairs × Clean ×2 × Ripple ×2.5 = 1,080`. Reduced motion gets the finished line; a screen reader hears the whole construction as one sentence. | Term tests against the rule at every tier and wave count, reveal-timing test, reduced-motion test. |
| **186** | The rung's worth on the meter, done (thesis T3.3). The meter showed where the player stood on the ladder and never what standing there bought. A cluster of pips beside the tier now carries the pairs a break takes at that rung - one pip each, growing as the player climbs - with the whole ladder in the hover hint and in the meter's accessible label. The numbers are measured (`simulatePopReach`: 1.91 / 3.19 / 3.50 / 6.95 pairs) and a test re-runs that simulation against the constants the HUD draws, so the promise cannot drift from the rule. Recorded with it: Sharp pays 0.32 pairs over Clean, against a band floor of 0.25 - the thin middle rung Gen 168 fixed has walked back, and the pips are what made it visible. | Rung-value test against the live simulation, HUD pip test, meter label tests. |
| **185** | The aim guide made honest, done (thesis T3.1/T3.2). `getClumpRead` had walked the whole connected region at infinite depth and reported `pairsSharpWouldTake` whatever chain the player was on - so the one affordance meant to teach the ladder was the single place the ladder was invisible, and at chain zero it promised a sweep the pop cannot make. It now previews by running `resolveChunkBreak` itself, at `momentum + 1` (the expression the live turn resolves with), so the promise and the rule cannot drift. The board draws two outlines: solid for what this match takes now, ghosted for what the next rung would add. The chip and the accessible name say both numbers ("Ember clump of 5 — this match pops 1 pair; 2 pairs at Clean."). Measured on the test fixture, the rungs read 1 / 2 / 4 / 7 pairs. | Ladder tests per rung, a preview-equals-rule test at five chains, accent tests, live-label test. |
...
| **Delve into the franchises and see what makes them tick.** | Section 1 above — Puzzle Bobble's leverage and drop, Peggle's multiplier ladder and last peg, Zuma's chains of chains, Bejeweled's gravity and Sugar Crush — and section 2's translation of each into a memory game, with 2.7 saying what was deliberately left out. The aim guide a bubble shooter draws became the clump read (Gen 127, `clump-read-rules.ts`), which follows the pointer and keyboard focus and, since Gen 185, names the pairs this match takes at the tier it would land on and ghosts what the next rung would add. |
...
Tuning Fork lends a lone match the chain's reach (partners leave and seed one wave) and adds a wave to Clean. A halo pair is the edge of the celebration, not a bridge: it does not seed. The matched pair itself is never re-taken by a later wave. The drop (Gen 137, restructured as the severance drop in Gen 180) fires after the last wave at any tier and is not a wave: a suit that can no longer pop loses its plain pairs. A break pays a pair's worth times the pairs, times the tier it landed at (none 1, Clean 2, Sharp 4, Fever 8), times the ripple (1 + 0.75 a wave past the first, capped at 6) - Gen 181, thesis §40.2; the size bonus, the Fever lift and the 0.2-a-wave ripple lift went with it. The floor's end pays 100 × floor times the tier still standing (Clean 1.5, Sharp 2.5, Fever 5) plus 50 × floor for every turn under the floor's par, `ceil(pairs × 0.4)` (§40.5, §41.3; the par constant is measured, see BALANCE_NOTES Gen 181). The floor clears in place (Gen 182, §41.4): the last pair leaves, a beat over the board says the floor, its turns against par, what it paid and why, and the next board builds on the same surface about 1.6 seconds later; there is no screen between floors.

The run ends two ways and no other (Gen 183, §42.2): the player stops, or a floor is not cleared within its turn ceiling, three times its par (`turnCeilingForFloor`, `floor-par.ts`), checked after every resolved turn - a floor that clears on its ceiling turn is a clear. There are no lives: a miss halves the chain, counts a try and a turn, and costs nothing else, so a bad floor is quiet - the chain resets, the floor pays less, the next one starts (§67, trace 4). What the hearts used to say the par stat says now, with the ceiling beside it and a mark on the last two turns. The life economy that went, and why, is in `docs/REMOVED_LIVES.md`.
...
| **196** | ~~The glass decoy, the last of the fake cards.~~ Removed with the `glass_floor` mutator and the `glass_witness` objective; `docs/REMOVED_DECOY.md` is the record. Nothing the cascade measures moved, which is the indictment rather than a reassurance. | A mechanic that costs nothing to remove was contributing nothing - while holding a Codex entry, a mutator slot, two `RunState` fields and a renderer prop chain through five components. |
| **197** | ~~A pop reaches only cards it actually touches.~~ Done: the partner reach and the Fever halo are gone; the ladder is rebuilt from reach (2 / 4), the reaction, and the **bridge** into a touching clump (1 at Sharp, 3 at Fever, plus diagonals). `docs/BREAK_TOUCHES_ONLY.md`. | Contact-only capped a break at one suit clump - three or four pairs here - and flattened the ladder to a spread of 1.21 with Sharp worth 0.10 over Clean. Depth cannot fix that because the clump runs out; the bridge can, and never crosses a gap. An **uncounted** bridge is not a rung, it is the board: unbounded, Sharp took 8.26 pairs. |
