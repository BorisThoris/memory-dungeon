...
**Finding:** **`graphicsQuality`**, **`boardBloomEnabled`**, **`boardScreenSpaceAA`** shipped in settings/contracts (**`PERF-001`**, **`PERF-002`**). Hidden-tab menu animation suspension (**`PERF-003`**), TileBoard context-loss recovery (**`PERF-005`**), menu Pixi DPR caps (**`PERF-006`**), and quality-tier anisotropy (**`PERF-007`**) are implemented and covered. OS `prefers-reduced-motion` is only partially used; remaining gaps are below.
