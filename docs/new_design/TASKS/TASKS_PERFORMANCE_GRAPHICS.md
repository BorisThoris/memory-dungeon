...
**Finding:** **`graphicsQuality`**, **`boardBloomEnabled`**, **`boardScreenSpaceAA`** shipped in settings/contracts (**`PERF-001`**, **`PERF-002`**). Hidden-tab menu animation suspension (**`PERF-003`**), effective saved/OS reduced motion (**`PERF-004`**), TileBoard context-loss recovery (**`PERF-005`**), menu Pixi DPR caps (**`PERF-006`**), quality-tier anisotropy (**`PERF-007`**), and native-AA-aware Canvas remounting (**`PERF-008`**) are implemented and covered.
