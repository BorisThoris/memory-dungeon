...
- **2026-09-24 misses are earned, and they expire (`miss-bank.ts`, `RunState.missBank`):** the player asked for lives won per floor or per combo, carried "to the next level plus three levels from that round", with a formula that is punishing but winnable. The bank is a ledger of grants stamped with the floor each was earned on: 3 to open (floor 1), +1 per floor cleared, +1 every five matches in a row (spendable at once), each good through floor N+3 and gone on N+4, never more than 4 in hand - at the cap a new grant pushes the oldest out rather than being lost. A miss spends the grant that would go first. Measured with `yarn sim:survival` (perfect-memory player, twenty seeds, median floor): 10% misses 45, 15% 17, 20% 6, 25% 6, 35% 4 - against 37/21/11/9/5 under the flat +2. The careful player goes deeper because chains feed the bank; everyone else is out sooner because floor 2's misses stop being paid for on floor 6. Tunings tried: a 6-chain rung put 15% at 13 and 10% at 32; +2 a floor at a 6-chain gave 18 and 49 but expired 1.2 a floor unspent, so the shelf never bit; a 4-chain rung let 10% never die. The other censuses still hold the bank open.
- **2026-09-23 the store is back, as a sheet on the pause menu (`run-store-rules.ts`):** the player asked for the old dungeon/store aspects with the current UI, and the miss bank gave a store its first honest reason: another miss is worth buying. Gold is a run currency earned only at a floor clear (2, +1 a rung cleared at, +1 a turn under par to 3) and spent on the pause menu - never a door between floors, which is what Gen 174 removed the shop for. Three items (a miss to the bank's cap, a peek, a shuffle), prices climbing per purchase in the run. Gold ends with the run. First slice; measured against the loop by the same censuses, which do not read the purse.
- **2026-09-23 the run's budget is misses, not turns (`miss-bank.ts`):** the turn bank read "17 left" on floor 4 because it counted the turns a floor needs to be matched at all; the player read it as seventeen lives and called it undeserved. Replaced by a miss bank - opens 3, +2 a floor, cap 4, a miss with none left ends the run (`miss_budget`) - and the turn ceiling removed. Measured with a perfect-memory player, twenty seeds, median floor reached: 10% misses 37, 15% 21, 20% 11, 25% 9, 35% 5; the 2/+1/3 tuning the ask literally read as put a 15% player at floor 11 and was too harsh. The cascade and curve sims hold the bank open like the censuses, because they measure the floor and not survival.
