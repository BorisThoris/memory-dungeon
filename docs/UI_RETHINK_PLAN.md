...
**Status: built.** Codex is one section rail (thirteen sections, one active), one filter that
searches every section, and one grid of entry cards (section kicker, title, one line). The stacked
fold panels, TOC links, "knowledge-base summary" tiles and reward-signal frame are gone. Inventory is one header line plus
