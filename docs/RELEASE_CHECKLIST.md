...
| No screen offers two controls a player reads as the same instruction | done | the repository | `scripts/control-label-ambiguity.ts` |
...
| Every card a match or a break takes leaves the board, and the survivors fall in toward the middle to close the gap | done | the repository | `src/shared/board-settle-rules.ts` |
