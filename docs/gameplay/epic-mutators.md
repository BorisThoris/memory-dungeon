...
| `sticky_fingers` | Blocks a face-down neighbour of the match on the next “opening” flip (it used to block the matched slot itself, which blocked nothing) | HUD chip; no on-grid blocked-slot art | **Functional** |
