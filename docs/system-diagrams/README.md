...
- Findings: 12
- Audit actions: 18
- Import graph: 941 files, 3211 edges
- Rules version: 33
...
Serialized run bootstrap, renderer intent, and historical shared state-in/state-out calls converge on one accepted-command journal transaction before pure rules; one command playthrough gate proves generated-board completion, feedback, fairness, and replay.

```mermaid
flowchart LR
    input["Player Input"]
    run_bootstrap["Run Bootstrap"]
    command_transaction["Command Transaction"]
    compatibility_facade["Compatibility Facade"]
    rules["Shared Rules"]
    command_playthrough["Command Playthrough Gate"]
    board_powers["Board Powers"]
    feedback["HUD Feedback"]
    progression["Run Progression"]
    terminal_cleanup["Terminal Interlude"]
    run_finalization["Run Finalization"]
    run_bootstrap -->|"initializes deterministic state"| rules
    run_bootstrap -->|"emits typed start cue"| feedback
    input -->|"submits serialized intent"| command_transaction
    compatibility_facade -->|"submits historical shared calls"| command_transaction
    command_transaction -->|"reduces and journals acceptance"| rules
    rules -->|"executes generated floors"| command_playthrough
    command_playthrough -->|"proves replayable completion"| progression
    rules -->|"checks legality"| board_powers
    rules -->|"publishes state"| feedback
    rules -->|"completes rooms"| progression
    progression -->|"guards dead interludes"| terminal_cleanup
    terminal_cleanup -->|"emits typed warning"| feedback
    rules -->|"submits terminal state"| run_finalization
    terminal_cleanup -->|"hands off cleaned terminal state"| run_finalization
    run_finalization -->|"feeds validated game-over summary"| feedback
```
...
- **Run Bootstrap** (domain/shared): A schema-validated command captures seed, time, mode, settings, and restart context before constructing run state. Evidence: `src/shared/run-start-core-contracts.ts`, `src/shared/run-start-core.ts`, `src/renderer/store/runStartState.ts`
- **Command Transaction** (domain/shared): One adapter reduces serialized intent and appends accepted commands plus ordered events to bounded journals. Evidence: `src/shared/gameplay-core-adapters.ts`, `src/shared/gameplay-journal.ts`, `src/shared/renderer-command-transaction-boundary.test.ts`
- **Compatibility Facade** (domain/shared): Historical shared flip, turn, Destroy, and exit calls retain their return shape but execute only through the command adapter; game.ts is an import-free barrel. Evidence: `src/shared/gameplay-command-compatibility.ts`, `src/shared/game-barrel-ownership-boundary.test.ts`, `src/shared/game.ts`
...
- **Command Playthrough Gate** (domain/shared): Generated-board properties, fairness consumers, and endless simulation use one replayable solver; the retired direct-mutation solver cannot return. Evidence: `src/shared/gameplay-core-playthrough-solver.ts`, `src/shared/playthrough-solver-rules.ts`, `src/shared/legacy-playthrough-solver-boundary.test.ts`
...
- **Terminal Interlude** (domain/shared): A strict command converts zero-life reward/shop/continuation states into journaled game over. Evidence: `src/shared/interlude-transition-rules.ts`, `src/shared/gameplay-core.ts`, `src/renderer/store/levelCompleteContinuationExecutor.ts`
- **Run Finalization** (domain/shared): A strict command validates the terminal summary and journals its own command/event before persistence. Evidence: `src/shared/gameplay-core-contracts.ts`, `src/shared/gameplay-core-adapters.ts`, `src/shared/run-summary-rules.ts`, `src/renderer/store/runResolutionController.ts`, `src/renderer/components/GameOverScreen.tsx`

## Gameplay Interaction Graph

Executable registry covers 145 mechanics, 681 edges, 9 tile traits, and 24 blockers.
...
- **INFO Cross-feature logic now has an executable graph.** Mechanics declare reads, writes, blockers, counterplay, softlock guards, evidence, and tests. Keep this registry current whenever adding traits, hazards, objectives, locks, powers, shops, or boss logic.
  Evidence: `src/shared/gameplay-interaction-graph-data.json`, `src/shared/gameplay-interaction-graph.ts`, `src/shared/gameplay-interaction-graph.test.ts`, `src/shared/softlock-fairness.test.ts`, `src/shared/tile-trait-rules.ts`, `src/shared/enemy-hazard-board-rules.ts`
- **WARNING Every blocker needs counterplay or a softlock guard.** The graph currently tracks 24 blocking mechanics. New blockers should fail validation unless they declare counterplay edges, tests, and softlock guards.
  Evidence: `src/shared/gameplay-interaction-graph-data.json`, `src/shared/gameplay-interaction-graph.ts`, `src/shared/gameplay-interaction-graph.test.ts`, `src/shared/softlock-fairness.test.ts`, `src/shared/tile-trait-rules.ts`, `src/shared/enemy-hazard-board-rules.ts`
