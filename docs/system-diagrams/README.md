...
- Findings: 12
- Audit actions: 18
- Import graph: 966 files, 3001 edges
- Rules version: 33
...
- **P1 Lock reward priority slots before adding fun offers** (Rewards And Economy, done): Any new reward or relic offer must prove it does not displace required keys, boss access, loadout recovery, or trait-route starter support. Command: `yarn gate:rewards-economy`.
  ...
  Evidence: `src/shared/bonus-rewards.ts`, `src/shared/relics.ts`, `src/shared/run-economy.ts`, `src/shared/balance-simulation.ts`
...
Executable registry covers 130 mechanics, 479 edges, 9 tile traits, and 21 blockers.
...
flowchart LR
    ...
    relic_offers["Relic Definitions"]
...
- **Relic Definitions** (domain/shared): Relic definitions and favor; the draft that handed them out went in Gen 175. Evidence: `src/shared/relics.ts`
...
- **Reward UI** (ui/renderer): Renderer shows pickable rewards. Evidence: `src/renderer/components`, `src/renderer/App.tsx`
