...
- **AI-first gameplay core:** Fifty-three vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v53 promotes Route Gambler to the fourth bounded-memory long-horizon build with a distinct `risk_conversion` axis: Route Tactician movement, identity-blind Gambit commits, actual featured-objective wagers, Wager Surety win/loss effects, Favor cash-out, visible route risk, and Greed/Mystery preference now form one replayable policy. The gate completes all 144 floors without rejected commands, records 9 Gambits and 19 wagers (13 wins, 5 losses), and completes 21 hazard-counter floors while suppressing unsafe Gambits in that matchup. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
