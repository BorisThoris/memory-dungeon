...
- **AI-first gameplay core:** Nine cohesive build slices now cross the strict command/event boundary: Conduit Cartographer, Warden, Combo Shard Engine, Saboteur, Vaultbreaker, Slayer, Seer, Route Gambler, and Board Tactician. Board Tactician connects mixed bonus rewards and shuffle relics to explicit charges/free floor uses, then journals full shuffle, selected-row shuffle, and selected-tile swap through the existing deterministic board rules; architecture and status live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
