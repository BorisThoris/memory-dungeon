...
- **AI-first gameplay core:** Thirteen cohesive build slices now cross the strict command/event boundary: Conduit Cartographer, Warden, Combo Shard Engine, Saboteur, Vaultbreaker, Slayer, Seer, Route Gambler, Board Tactician, Memory Scout, Locksmith, Combo Shard Survival, and the Supply Cache Emergency Toolkit. The latest slice makes Supply Cache a typed bridge from Peek-based inspection into completion-safe Destroy Pair recovery, with score, journal, feedback, simulation, and graph coverage; architecture and status live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
