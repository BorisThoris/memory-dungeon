...
- **AI-first gameplay core:** Seventeen cohesive gameplay slices now cross the strict command/event boundary: Conduit Cartographer, Warden, Combo Shard Engine, Saboteur, Vaultbreaker, Slayer, Seer, Route Gambler, Board Tactician, Memory Scout, Locksmith, Combo Shard Survival, Supply Cache Emergency Toolkit, Wild Joker Token, typed Destroy Pair, Hazard Banish Floor Payoff, and typed Route Choice. A separate semantic slice models relic, mutator, and contract loadouts as read-only projections over authoritative run state. The latest command slice journals the selected Safe, Greed, or Mystery route plus exact progression and economy deltas before continuation; the balance profiler now executes that same command and scores its typed outcomes instead of duplicating reward arithmetic. Architecture and status live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
