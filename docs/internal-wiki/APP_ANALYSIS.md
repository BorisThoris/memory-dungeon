...
- **AI-first gameplay core:** Seventy-two vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v72 keeps all 127 active `RunState` fields plus all 34 Zod command variants and 54 event variants compiler-indexed, rejects production renderer state writes, and enforces source-derived orchestration budgets. Six ratchets extract match/mismatch lanes, route/onboarding/Gambit/wager signals, floor-clear feedback, recall-aware route choice, board-floater orchestration, and its renderer projection from `GameScreen`, cutting it from 6,812 to 4,456 physical lines and lowering its ceiling to 4,500 modeled lines/60 imports. The 15,776-relationship model retains 27 player-visible states with zero ownership, orchestration, protocol, or feedback diagnostics, while the nine-build gate still completes all 324 floors with zero rejected commands and exact replay. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
