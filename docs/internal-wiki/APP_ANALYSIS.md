...
- **AI-first gameplay core:** Sixty-six vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v66 keeps all 127 active `RunState` fields plus all 34 Zod command variants and 54 event variants compiler-indexed, classifies direct assignment versus complete state construction, and rejects production renderer writes to core-owned run state. Settings application, dead-interlude normalization, and terminal summary construction now live in shared deterministic rules; React/Zustand route returned state instead of manufacturing it. The 15,570-relationship model retains 27 player-visible states and zero diagnostics, while the nine-build gate still completes all 324 floors with zero rejected commands and exact replay. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
