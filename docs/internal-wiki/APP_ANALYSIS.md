...
- **AI-first gameplay core:** Sixty vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v60 consolidates the redundant Seer declaration into the retained Conduit Cartographer information-control build. Echo/Conduit sources and Peek now share one loop with 9 real Scout Glint matches, deterministic threat reveals, Memory Nail, and 46 typed known-pair Pin placements; Pin automation is suppressed on all 13 hazard-pressure floors. The nine-build gate still completes all 324 floors with zero rejected commands and exact replay. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
