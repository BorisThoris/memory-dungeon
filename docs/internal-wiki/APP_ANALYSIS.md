...
- **AI-first gameplay core:** Fifty-two vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v52 adds visible-resource interlude risk to the bounded-memory 108-floor counter-matchup gate: Conduit Cartographer, Warden, and Vaultbreaker preview typed route outcomes, spend explicit risk caps against life/guard/ward/parasite/Destroy protection, rank actual deterministic event resources, retain explainable assessments, and replay every floor and seed exactly. The gate records 240 route assessments, 48 policy rejections, one adaptive route switch, no rejected gameplay commands, and names Route Gambler—not yet long-horizon sampled—as the next `risk_conversion` build gap. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
