...
- **AI-first gameplay core:** Fourteen cohesive gameplay slices now cross the strict command/event boundary: Conduit Cartographer, Warden, Combo Shard Engine, Saboteur, Vaultbreaker, Slayer, Seer, Route Gambler, Board Tactician, Memory Scout, Locksmith, Combo Shard Survival, Supply Cache Emergency Toolkit, and Wild Joker Token. The latest slice makes each Wild match a validated one-token spend, journals the actual paired tiles, preserves additional tokens, and regenerates a joker on later floors while balance remains; architecture and status live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
