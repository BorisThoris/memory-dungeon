...
- **AI-first gameplay core:** Fifteen cohesive gameplay slices now cross the strict command/event boundary: Conduit Cartographer, Warden, Combo Shard Engine, Saboteur, Vaultbreaker, Slayer, Seer, Route Gambler, Board Tactician, Memory Scout, Locksmith, Combo Shard Survival, Supply Cache Emergency Toolkit, Wild Joker Token, and typed Destroy Pair. A separate semantic slice models relic, mutator, and contract loadouts as read-only projections over authoritative run state. The latest command slice journals exact pair removal, charge/focus/parasite/spotlight deltas, feedback, replay, and the board-complete handoff before the established finalizer runs; architecture and status live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
