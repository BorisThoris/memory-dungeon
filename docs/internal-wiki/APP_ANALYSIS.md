...
- **AI-first gameplay core:** Twenty-three cohesive gameplay slices now cross the strict command/event boundary: Conduit Cartographer, Warden, Combo Shard Engine, Saboteur, Vaultbreaker, Slayer, Seer, Route Gambler, Board Tactician, Memory Scout, Locksmith, Combo Shard Survival, Supply Cache Emergency Toolkit, Wild Joker Token, typed Destroy Pair, Hazard Banish Floor Payoff, typed Route Choice, typed Relic Draft Selection, typed Relic Offer Services, typed Route Side Rooms, flat typed Floor Advancement, and typed non-final Board Turn Resolution. A separate semantic slice models relic, mutator, and contract loadouts as read-only projections over authoritative run state. Route balance profiles execute the live route command; typed between-floor commands and ordinary non-final matches now journal one outer command with ordered domain events rather than nested effect-command journals. Final-pair floor clearing remains an explicit compatibility boundary. Architecture and status live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
