...
- **AI-first gameplay core:** Forty-nine vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v49 routes delayed board-turn resolution directly from the renderer through one typed command adapter with adapter-owned journaling and exact current-command feedback events. Transient Stray intent ownership and the typed-only renderer boundary still cover ordinary, Gambit, Destroy, exit, shop, and room presses, while the three-build multi-floor gate proves Conduit Cartographer, Warden, and Vaultbreaker across generated floors, recurring synergies, observed pressure distributions, and exact replay. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
