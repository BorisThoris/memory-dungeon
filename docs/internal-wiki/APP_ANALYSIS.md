...
- **AI-first gameplay core:** Sixty-one vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v61 derives one 27-field player-visible registry from the deterministic feedback facts and uses it in runtime command audits, graph diagnostics, and AI-model state metadata. The widened gate found and connected missing typed-key and Flash-charge HUD relationships; the 10,709-relationship model has zero diagnostics. The nine-build gate still completes all 324 floors with zero rejected commands and exact replay. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
