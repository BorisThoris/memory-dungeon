...
- **AI-first gameplay core:** Fifty-five vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v55 retains the five-build/180-floor gate and closes a Trap Control ownership break: Route Tactician and Free Swap Discipline's renewable perk now legally funds typed row shuffle or tile swap, consumes the free use before paid charges, and projects the same availability through player teaching. This establishes the command-correct base for the sixth `board_reconfiguration` policy. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
