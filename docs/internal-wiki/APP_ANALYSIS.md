...
- **AI-first gameplay core:** Fourteen cohesive gameplay slices now cross the strict command/event boundary: Conduit Cartographer, Warden, Combo Shard Engine, Saboteur, Vaultbreaker, Slayer, Seer, Route Gambler, Board Tactician, Memory Scout, Locksmith, Combo Shard Survival, Supply Cache Emergency Toolkit, and Wild Joker Token. A fifteenth semantic slice models relic, mutator, and contract loadouts as read-only projections over authoritative run state, connecting setup, draft/schedule mutation windows, build composition, power restrictions, and HUD visibility without inventing duplicate commands; architecture and status live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
