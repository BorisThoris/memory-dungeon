...
- **AI-first gameplay core:** Sixty-four vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v64 adds a compiler-derived ownership index for all 127 active `RunState` fields with exact declaration/read/write references and rejects fields without production readers. Its first audit removed `dailyStreakCount`, duplicated shop-visit metadata, and duplicated wild-tile metadata; authoritative board and profile behavior is unchanged. The 14,497-relationship model retains 27 player-visible states and zero diagnostics, while the nine-build gate still completes all 324 floors with zero rejected commands and exact replay. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
