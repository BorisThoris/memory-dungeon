...
- **AI-first gameplay core:** Forty-six vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v46 adds a three-build multi-floor gate: Conduit Cartographer, Warden, and Vaultbreaker each clear four generated floors across three seeds with real trait-synergy evidence, observed pressure distributions, bounded turn ratios, per-floor replay checkpoints, and exact full-run replay. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
