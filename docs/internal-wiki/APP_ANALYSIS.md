...
- **AI-first gameplay core:** Sixty-seven vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v67 keeps all 127 active `RunState` fields plus all 34 Zod command variants and 54 event variants compiler-indexed, rejects production renderer state writes, and adds source-derived orchestration budgets. The first ratchet extracts the pure match/mismatch lane model from `GameScreen`, cutting the component from 6,812 to 6,553 physical lines and enforcing a 6,600-line/62-import ceiling. The 15,609-relationship model retains 27 player-visible states with zero ownership, orchestration, protocol, or feedback diagnostics, while the nine-build gate still completes all 324 floors with zero rejected commands and exact replay. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
