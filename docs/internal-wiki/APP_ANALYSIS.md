...
- **AI-first gameplay core:** Fifty-seven vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v57 promotes Boss Hunter to the seventh bounded-memory long-horizon build with a distinct `boss_extraction` axis. Chapter Compass trophy conversion, Wager Surety objective insurance, Parasite Ledger relief, boss-pressure payoff, and parasite-counter exposure form one replayable policy. The seven-build gate completes all 252 floors; Boss Hunter records 12 trophy conversions, 3 relief events, and 19 wagers with 13 wins and 5 losses while retaining a 1.12 worst pairwise pace ratio. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
