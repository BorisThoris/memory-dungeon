...
- **AI-first gameplay core:** Sixty-three vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v63 keeps the 27-field player-visible registry source-derived and completes direct board-power intent ownership: Stray arming is renderer-transient, while Region Shuffle carries its selected row directly in the typed command with no durable arm field. The 10,703-relationship model has zero diagnostics, and the nine-build gate still completes all 324 floors with zero rejected commands and exact replay. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
