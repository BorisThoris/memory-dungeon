...
- **AI-first gameplay core:** Sixty-two vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v62 keeps the 27-field player-visible registry source-derived and completes Stray Remove ownership: durable `RunState` holds charges and consequences, while renderer-only surface state owns arming. The 10,706-relationship model has zero diagnostics, and the nine-build gate still completes all 324 floors with zero rejected commands and exact replay. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
