...
- **AI-first gameplay core:** Forty-eight vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v48 adds transient surface-intent ownership: live Stray Remove arming is application state, its typed command is independent of the legacy serialized arm flag, and the unused Region Shuffle renderer arm action is gone. The typed-only renderer boundary still covers ordinary, Gambit, Destroy, exit, shop, and room presses, while the three-build multi-floor gate proves Conduit Cartographer, Warden, and Vaultbreaker across generated floors, recurring synergies, observed pressure distributions, and exact replay. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
