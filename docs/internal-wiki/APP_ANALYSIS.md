...
- **AI-first gameplay core:** Fifty-six vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v56 promotes Saboteur to the sixth bounded-memory long-horizon build with a distinct `board_reconfiguration` axis. Route Tactician, Trait Toolkit, Free Swap Discipline, targeted row shuffle, hazard-pressure use, and memory-pressure conservation form one replayable policy. The six-build gate completes all 216 floors; Saboteur records 12 reconfigurations, 6 counter conservations, 33 rejected over-budget route options, and a 1.12 worst pairwise pace ratio. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
