...
- **AI-first gameplay core:** Forty-seven vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v47 also enforces a typed-only renderer board-input boundary: ordinary, Gambit, Destroy, exit, shop, and room presses use explicit command adapters, and a source gate prevents legacy mutation imports from returning. The three-build multi-floor gate still proves Conduit Cartographer, Warden, and Vaultbreaker across generated floors, recurring synergies, observed pressure distributions, and exact replay. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
