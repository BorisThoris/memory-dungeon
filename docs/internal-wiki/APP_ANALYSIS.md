...
- **AI-first gameplay core:** Twenty-four cohesive gameplay slices now cross the strict command/event boundary: Conduit Cartographer, Warden, Combo Shard Engine, Saboteur, Vaultbreaker, Slayer, Seer, Route Gambler, Board Tactician, Memory Scout, Locksmith, Combo Shard Survival, Supply Cache Emergency Toolkit, Wild Joker Token, flat typed Destroy Pair, Hazard Banish Floor Payoff, typed Route Choice, typed Relic Draft Selection, typed Relic Offer Services, typed Route Side Rooms, flat typed Floor Advancement, and flat typed Board Turn Resolution. A separate semantic slice models relic, mutator, and contract loadouts as read-only projections over authoritative run state. Route balance profiles execute the live route command; typed between-floor commands, ordinary/Gambit turns, and completion-causing Destroy actions now journal one outer command with ordered domain events. Dungeon Exit finalization remains the next explicit seam. Architecture and status live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
