...
- **AI-first gameplay core:** Fifty-nine vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v59 promotes Locksmith to the ninth bounded-memory long-horizon build with a distinct `lock_extraction` axis. Vaultbreaker and Key Insurance stock typed keys; the opt-in policy visits real board vendors through typed pause/purchase/resume, prefers matching keys, falls back to Master Key, opens affordable locked caches or alternate exits, and conserves access under hazard pressure while leaving the primary exit safe. The nine-build gate completes all 324 floors with exact replay; Locksmith records 8 typed-key lock uses, 1 Master Key lock use, 7 Master Key purchases, and 13 counter-pressure conservation floors across 36 completed floors. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
