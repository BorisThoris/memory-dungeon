...
- **AI-first gameplay core:** Seventy vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v70 keeps all 127 active `RunState` fields plus all 34 Zod command variants and 54 event variants compiler-indexed, rejects production renderer state writes, and enforces source-derived orchestration budgets. Four ratchets extract match/mismatch lanes, route/onboarding/Gambit/wager decision signals, floor-clear cashout/carry-forward feedback, and the recall-aware route-choice decision model from `GameScreen`, cutting it from 6,812 to 5,308 physical lines and lowering its ceiling to 5,350 modeled lines/62 imports. The 15,716-relationship model retains 27 player-visible states with zero ownership, orchestration, protocol, or feedback diagnostics, while the nine-build gate still completes all 324 floors with zero rejected commands and exact replay. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
