...
- **AI-first gameplay core:** Fifty vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v50 adds exported strategy policies and a 108-floor shipped counter-matchup gate: Conduit Cartographer, Warden, and Vaultbreaker make legality-checked route/reward/relic/shop decisions, encounter declared favorable and counter pressures, and replay every floor and seed exactly. The longer sweep also closed typed feedback for final-pair enemy blockers defeated during tile flips. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
