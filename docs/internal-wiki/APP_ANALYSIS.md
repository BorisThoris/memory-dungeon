...
- **AI-first gameplay core:** Sixty-five vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v65 keeps all 127 active `RunState` fields compiler-indexed and adds first-class nodes for all 34 Zod command variants and 54 event variants, including exact payload, creator, handler, emitter, consumer, independent-test, journal, and renderer-display relationships. `effects.apply` is an explicit exhaustive reducer branch, and missing handlers, emitters, protocol tests, or feedback display consumers are build errors. The 15,570-relationship model retains 27 player-visible states and zero diagnostics, while the nine-build gate still completes all 324 floors with zero rejected commands and exact replay. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
