...
- **AI-first gameplay core:** Fifty-four vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v54 promotes Combo Shard Engine to the fifth bounded-memory long-horizon build with a distinct `sustain_conversion` axis: Vaultbreaker, Bonus Shards, Combo Shard + Step, Shard Spark, a visible full-bank Greed credit, authoritative third-shard life conversion, and parasite-pressure exposure now form one replayable policy. The five-build gate completes all 180 floors without rejected commands; Combo Shard Engine records 18 source events, 22 life conversions, 6 adaptive route selections, 2 favorable economy floors, and 3 parasite-counter floors. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
