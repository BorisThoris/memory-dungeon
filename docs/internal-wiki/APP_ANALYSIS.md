...
- **AI-first gameplay core:** Seventy-one vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v71 keeps all 127 active `RunState` fields plus all 34 Zod command variants and 54 event variants compiler-indexed, rejects production renderer state writes, and enforces source-derived orchestration budgets. Five ratchets extract match/mismatch lanes, route/onboarding/Gambit/wager decision signals, floor-clear cashout/carry-forward feedback, recall-aware route choice, and board-floater orchestration from `GameScreen`, cutting it from 6,812 to 4,723 physical lines and lowering its ceiling to 4,750 modeled lines/61 imports. The 15,763-relationship model retains 27 player-visible states with zero ownership, orchestration, protocol, or feedback diagnostics, while the nine-build gate still completes all 324 floors with zero rejected commands and exact replay. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
