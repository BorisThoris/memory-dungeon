...
- **AI-first gameplay core:** Sixteen cohesive gameplay slices now cross the strict command/event boundary: Conduit Cartographer, Warden, Combo Shard Engine, Saboteur, Vaultbreaker, Slayer, Seer, Route Gambler, Board Tactician, Memory Scout, Locksmith, Combo Shard Survival, Supply Cache Emergency Toolkit, Wild Joker Token, typed Destroy Pair, and Hazard Banish Floor Payoff. A separate semantic slice models relic, mutator, and contract loadouts as read-only projections over authoritative run state. The latest command slice journals deterministic floor-start hazard removal, fallback Destroy gain, or explicit contract suppression before play begins; architecture and status live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
