...
- **AI-first gameplay core:** Fifty-one vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v51 adds bounded-memory board policies to the 108-floor shipped counter-matchup gate: Conduit Cartographer, Warden, and Vaultbreaker retain capped observation ledgers, spend explicit uncertain-turn budgets, make legality-checked route/reward/relic/shop decisions, encounter declared favorable and counter pressures, and replay every floor and seed exactly. The gate completes all 108 floors without rejected commands or budget exhaustion while preserving perfect-information mode only for legacy parity/fairness consumers. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
