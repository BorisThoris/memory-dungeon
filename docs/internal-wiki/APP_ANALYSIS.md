...
- **AI-first gameplay core:** Fifty-eight vertical slices now cover the primary deterministic loop: input, turns, utility powers, exits, floor completion, route/side-room/shop/relic interludes, floor advancement, pause/debug lifecycle, safety repair, event-only feedback, persistence, replay, and generated-board fairness. Graph v58 promotes Memory Scout to the eighth bounded-memory long-horizon build with a distinct `mistake_recovery` axis. Study relics, Trait Streak Lens/Toolkit, Flash Pair, and identity-blind uncertain-mismatch Undo form one replayable policy, with boss-special Undo suppression preserving solver safety. The eight-build gate completes all 288 floors with exact replay; Memory Scout records 6 Flash and 11 Undo uses across 36 completed floors, and the suite retains a 1.12 worst pairwise pace ratio. Architecture and current gaps live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
