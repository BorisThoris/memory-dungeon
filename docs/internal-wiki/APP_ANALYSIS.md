...
- **AI-first gameplay core:** Twenty cohesive gameplay slices now cross the strict command/event boundary: Conduit Cartographer, Warden, Combo Shard Engine, Saboteur, Vaultbreaker, Slayer, Seer, Route Gambler, Board Tactician, Memory Scout, Locksmith, Combo Shard Survival, Supply Cache Emergency Toolkit, Wild Joker Token, typed Destroy Pair, Hazard Banish Floor Payoff, typed Route Choice, typed Relic Draft Selection, typed Relic Offer Services, and typed Route Side Rooms. A separate semantic slice models relic, mutator, and contract loadouts as read-only projections over authoritative run state. Route balance profiles execute the live route command; typed between-floor commands now journal route commitment, side-room rest/event/reward resolution, draft shaping, each selected relic, and deterministic continuation without nested command journals. Architecture and status live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
