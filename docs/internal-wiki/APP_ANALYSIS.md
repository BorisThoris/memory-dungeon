...
- **AI-first gameplay core:** Nineteen cohesive gameplay slices now cross the strict command/event boundary: Conduit Cartographer, Warden, Combo Shard Engine, Saboteur, Vaultbreaker, Slayer, Seer, Route Gambler, Board Tactician, Memory Scout, Locksmith, Combo Shard Survival, Supply Cache Emergency Toolkit, Wild Joker Token, typed Destroy Pair, Hazard Banish Floor Payoff, typed Route Choice, typed Relic Draft Selection, and typed Relic Offer Services. A separate semantic slice models relic, mutator, and contract loadouts as read-only projections over authoritative run state. Route balance profiles execute the live route command, while the latest command slices journal draft shaping, each selected relic, its immediate build effect, and deterministic multi-pick/final-offer consequence before continuation. Architecture and status live in [`.ai/GAMEPLAY_MIGRATION.md`](../../.ai/GAMEPLAY_MIGRATION.md).
