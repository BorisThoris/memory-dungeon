...
test.describe('Board 3D value', () => {
    test.describe.configure({ retries: 0 });

    for (const viewport of BOARD_3D_VALUE_VIEWPORTS) {
        test(`${viewport.name} renders a nonblank bounded board stage`, async ({ page }, testInfo) => {
            ...
            await testInfo.attach(`${viewport.name}-board-stage.png`, { body: shot, contentType: 'image/png' });
