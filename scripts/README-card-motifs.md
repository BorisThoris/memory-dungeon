...
The `src/renderer/assets/cards/illustrations/` folder may still hold SVG/PNG references for tooling or future hybrid loot rolls; PNG face-panel files are source masters, while `face-panel-NN.webp` files are the runtime imports generated by `yarn face-panels:export-runtime-webp`.
