...
describe('the momentum bonus ladder', () => {
    it('names the tier the momentum holds on this floor, and nothing below Clean', () => {
        // Twelve pairs: Sharp from 7, Fever from 9. The ladder pays nothing itself any more: gold
        ...
        expect(getFloorClearMomentumBonus({ chain: 5, cascadedPairs: 2, pairsOnFloor: 12 })).toEqual({ momentum: 7, tier: 'sharp' });
        expect(getFloorClearMomentumBonus({ chain: 5, cascadedPairs: 4, pairsOnFloor: 12 })).toEqual({ momentum: 9, tier: 'fever' });
    });
});
...
describe('Extreme Fever at the floor clear', () => {
    ...
    it('names the standing momentum tier, tags the result, and writes the chain recap', () => {
        const { run, board } = clearedRun(5, 4);
        ...
        expect(cleared.lastLevelResult?.chainMomentumAtClear).toBe(9);
        ...
        expect(cleared.lastLevelResult?.chunkPairsBroken).toBe(4);
    ...
    it('records the floor at the rung it paid, not at what the streak alone reached', () => {
        // Streak 5 on twelve pairs is short of Sharp (7); with four cascaded pairs the ladder
    ...
    it('pays nothing when the chain dropped before the last pair; a Fever finish multiplies the floor bonus and never the rating', () => {
        ...
        const fever = finalizeLevel(clearedRun(5, 4).run, board);
