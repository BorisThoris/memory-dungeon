...
describe('session-stats-rules', () => {
    ...
    it('normalizes partial trait counts and counts each trait pair once', () => {
        ...
        const counts = addTileTraitCountStats(
            { echo: 2 },
            [
                { ...heavyA, tileTraitKind: 'heavy' },
                { ...heavyB, tileTraitKind: 'heavy' },
    ...
    it('normalizes malformed trait counts and session stats', () => {
        // A counter for a trait that no longer exists is dropped rather than carried as a zero.
        expect(normalizeTileTraitCountStats({ echo: 2.8, heavy: Number.NaN, drift: 4 })).toEqual({
            echo: 2,
            heavy: 0,
            conduit: 0,
            stasis: 0
