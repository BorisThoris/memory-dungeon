...
describe('gameplay interaction graph', () => {
    ...
    it('connects concrete progression safety repairs to commands, feedback, persistence, and replay', () => {
        ...
        expect(gameplayInteractionGraph.version).toBe(53);
    ...
    it('evaluates route profiles and four distinct builds through long-horizon policies and shipped counter-matchups', () => {
        const byId = new Map(gameplayInteractionGraph.mechanics.map((mechanic) => [mechanic.id, mechanic]));
        expect(byId.get('simulation.build_evaluation')).toMatchObject({
            kind: 'simulation',
            role: 'route_profile_and_twelve_floor_four_build_bounded_memory_visible_resource_interlude_risk_conversion_policy_counter_matchup_viability_balance_feedback_and_replay_gate',
            evidence: expect.arrayContaining([
