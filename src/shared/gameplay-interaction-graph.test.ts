...
describe('gameplay interaction graph', () => {
    ...
    it('connects concrete progression safety repairs to commands, feedback, persistence, and replay', () => {
        ...
        expect(gameplayInteractionGraph.version).toBe(55);
    ...
    it('evaluates route profiles and five distinct builds through long-horizon policies and shipped counter-matchups', () => {
        const byId = new Map(gameplayInteractionGraph.mechanics.map((mechanic) => [mechanic.id, mechanic]));
        expect(byId.get('simulation.build_evaluation')).toMatchObject({
            kind: 'simulation',
            role: 'route_profile_and_twelve_floor_five_build_bounded_memory_visible_resource_interlude_risk_and_sustain_conversion_policy_counter_matchup_viability_balance_feedback_and_replay_gate',
            evidence: expect.arrayContaining([
