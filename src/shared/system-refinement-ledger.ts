 ...
 *
 * Gen 261. Gen 213 set this constant because the ledger recorded *what* had been examined and never
 * *when relative to the game*, and wrote the rule the walk is held to: "An entry written before any
 * of that is not evidence about this game; it is evidence about a game that used to be here,
 * indistinguishable from the real thing by reading."
 *
 * By its own rule it was due. Forty-six of the forty-eight entries still said Gen 213, and in the
 * generations since, par gave the opening a turn (220), stopped being one rate for every palette
 * (259) and started being read off the board rather than the floor number; the recovery floor's deal
 * changed (260); and the in-run chrome was rebuilt three times over (239, 240, 257, 258). The gate
 * passed the whole time, because it only ever asked whether the stamp was at or after the constant.
 *
 * So the gate requires every entry to be stamped at or after this constant. Raising it is the act of
 * re-walking all forty-eight, and the evidence fields below are what stop that walk from being a
 * stamp: raising the constant without looking leaves the numbers and the greps to fail.
 *
 * What the Gen 261 walk re-ran rather than re-read: the softlock sweep (430/430 playable, 0 fairness
 * issues), the endless health check (430 sampled floors, 0 issue floors, 997 trait floors, 0 dead),
 * the 384-step core replay (deterministic, no invariant violations), both occupancy censuses, the
 * eleven-key sampled-audio manifest, the ten mutator ids, and the one remaining singleton key. What
 * it found wrong: eight notes, listed in their own entries - a miss-rate ceiling quoted 0.012 high
 * and a margin four times narrower than stated, a share mislabelled as the miss rate, a HUD entry
 * that stopped forty-nine generations short, and three floor-identity surfaces naming a palette the
 * board does not deal.
 */
export const SYSTEM_REFINEMENT_SWEEP_GENERATION = 261;
...
export const SYSTEM_REFINEMENT_LEDGER: readonly SystemRefinementEntry[] = [
    ...
    },
    {
        ...
        verdict: 'confirmed',
        generation: 261,
        note: 'Re-measured 2026-09-23 after the pop was capped by rung (docs/CHAIN_CHUNK_FEVER_DESIGN.md §13): 0.988 x 2.79, the drop is the last-pair mercy and fires on nearly every floor, banded core. Before: The pop takes only cards in contact (Gen 197), and the drop fires on 0.397 of chain-one matches at 1.38 pairs a drop in sim:pop. The census reads the same mechanism from the other side: the drop takes a severed suit on 0.854 of floors. Gen 260 moved it from 0.871 with the recovery floor’s two-suit deal - a narrow palette leaves fewer suits to sever, and a floor that ends sooner gives the drop fewer chances. Both are measurements rather than the rule being read back. Re-measured 2026-09-25 from 0.988 x 2.80 when sticky fingers (floor 7) began locking a face-down card beside a match instead of the matched card, and the reference players began turning a locked card second, as a player who sees the lock does.'
    },
    {
    ...
    },
    {
        ...
        verdict: 'confirmed',
        generation: 261,
        note: 'Re-measured 2026-09-23 after the pop was capped by rung (docs/CHAIN_CHUNK_FEVER_DESIGN.md §13): 0.667 x 0.67, more turns a floor is more resolving pairs to undo. Before: Occupancy 0.467 x 0.47, banded common, and correctly bounded: undo only exists while a pair is resolving, so it cannot exceed the miss rate by much. The reference miss rate is 0.471 of floors, which is the ceiling it sits just under. Gen 261 re-measured it: the note had said 0.483, so the margin it describes was reported as 0.016 when it is 0.004 - undo is now within a rounding step of the rate that bounds it, which is worth knowing before anything else moves the miss rate. Re-measured 2026-09-25 from 0.671 x 0.67 when sticky fingers (floor 7) began locking a face-down card beside a match instead of the matched card, and the reference players began turning a locked card second, as a player who sees the lock does.'
    },
    {
        id: 'power.gambit',
        counter: 'gambit',
        verdict: 'confirmed',
        generation: 261,
        note: 'Re-measured 2026-09-23 after the pop was capped by rung (docs/CHAIN_CHUNK_FEVER_DESIGN.md §13): 0.642 x 0.64, more turns a floor is more first misses to spend the third flip on. Before: Occupancy 0.458 x 0.46, banded common. One third flip per floor, spent on the first miss, which is the only moment it can be spent - so its share tracks the chance a floor has a miss at all. Gen 262 re-measured it from 0.671 x 0.67 after a miss on a Heavy card began costing two from the bank: floors that meet Heavy end sooner. Re-measured 2026-09-25 from 0.654 x 0.65 when sticky fingers (floor 7) began locking a face-down card beside a match instead of the matched card, and the reference players began turning a locked card second, as a player who sees the lock does.'
    },
    {
        id: 'power.wild_match',
        counter: 'wildMatch',
    ...
    },
    {
        ...
        note: 'A guarantee rather than an occurrence, so its gate is the softlock seed sweep - 0 issue floors across the endless health check - not an occupancy row. Re-run at Gen 261 against the par Gens 220 and 259 set and the palette Gen 260 dealt: 430 of 430 playable floors and 0 fairness issues on every seed in the sweep, which is the first time this has been checked since par stopped being one rate for every palette.'
    },
    {
    ...
    },
    {
        ...
        note: 'Twenty-one branches of the in-run feedback rail watched for announcements the game cannot produce - guard caches, patrol paths, shop gold. Removed, and gated so no branch can outlive its announcement. Gen 212 then checked the rail the other way, laying the HUD out around the seven-digit score a sixty-floor run really carries rather than the three digits floor one produces. Gen 261 brought the entry up to the chrome that shipped after it, because the rail was the most-changed surface in the game and this note stopped forty-nine generations short: the chain HUD overlapped at 1.1 (Gen 239), the chain rail was drawn over the floor-clear beat at the same scale (Gen 240, 257 - the box never contained its own read), and the dock declared a toolbar it had wired none of (Gen 258). All three were painted-versus-layout px, all three are now measured by the fit contract and gate:controller at every scale the slider offers.'
    },
    {
        ...
        note: 'A tool for verifying the game rather than a rule in it. Its 384-step run stays deterministic and schema-clean after the command slots were compacted, and Gen 261 re-ran it against the par Gen 259 made palette-aware and the deal Gen 260 changed: 384 steps, replayDeterministic true, no invariant violations - so the determinism is the simulation rather than the boards it happened to be given.'
    },
    {
