...
export const SYSTEM_REFINEMENT_LEDGER: readonly SystemRefinementEntry[] = [
    ...
    },
    {
        ...
        verdict: 'confirmed',
        generation: 261,
        note: 'Re-measured 2026-09-23 after the pop was capped by rung (docs/CHAIN_CHUNK_FEVER_DESIGN.md §13): 0.671 x 0.67, more turns a floor is more resolving pairs to undo. Before: Occupancy 0.467 x 0.47, banded common, and correctly bounded: undo only exists while a pair is resolving, so it cannot exceed the miss rate by much. The reference miss rate is 0.471 of floors, which is the ceiling it sits just under. Gen 261 re-measured it: the note had said 0.483, so the margin it describes was reported as 0.016 when it is 0.004 - undo is now within a rounding step of the rate that bounds it, which is worth knowing before anything else moves the miss rate.'
    },
    {
        id: 'power.gambit',
        counter: 'gambit',
        verdict: 'confirmed',
        generation: 261,
        note: 'Re-measured 2026-09-25 once sticky fingers blocked a card that could still be played: 0.642 x 0.64. Before: Re-measured 2026-09-23 after the pop was capped by rung (docs/CHAIN_CHUNK_FEVER_DESIGN.md §13): 0.654 x 0.65, more turns a floor is more first misses to spend the third flip on. Before: Occupancy 0.458 x 0.46, banded common. One third flip per floor, spent on the first miss, which is the only moment it can be spent - so its share tracks the chance a floor has a miss at all. Gen 262 re-measured it from 0.671 x 0.67 after a miss on a Heavy card began costing two from the bank: floors that meet Heavy end sooner.'
    },
    {
        id: 'power.wild_match',
        counter: 'wildMatch',
