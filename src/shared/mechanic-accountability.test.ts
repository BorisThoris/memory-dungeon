...
describe('every mechanic answers for itself', () => {
    ...
    it('holds every mechanic in the graph to one of the two answers', () => {
        const answered = new Set([...Object.keys(MECHANIC_CENSUS_COUNTERS), ...Object.keys(MECHANIC_CENSUS_EXEMPTIONS)]);
    ...
    it('records how much of the game the census can actually see', () => {
        ...
        expect(censused).toBe(7);
        expect(gameplayInteractionGraph.mechanics.length).toBe(45);
        const blindToTheReferencePlayer = gameplayInteractionGraph.mechanics.filter((mechanic) =>
            /^(power|inventory|trait)\./u.test(mechanic.id)
        ).length;
        expect(blindToTheReferencePlayer).toBe(26);
    });
});
