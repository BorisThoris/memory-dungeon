...
| `ui/backgrounds/bg-gameplay-arcane-workshop-v1.webp` (`.png` master) | Gameplay stage under board | Local **Z-Image-Turbo** batch (`yarn ui-art:local`, take in `ui-backgrounds.zimage.picks.json`), then `yarn assets:ui-backgrounds:export-runtime-webp` | Arcane workshop vault; dark empty rune circle at center under the board. Install re-applies the previous master's alpha profile (`scripts/card-pipeline/masks/*.alpha.png`) so the theme gradients still show through the center. |
| `ui/backgrounds/bg-board-arcane-table-v1.webp` (`.png` master) | Gameplay board-stage workbench texture | Local **Z-Image-Turbo** batch (`yarn ui-art:local`, take in `ui-backgrounds.zimage.picks.json`), then `yarn assets:ui-backgrounds:export-runtime-webp` | Top-down slate slab with a faint etched rune ring. Install re-applies the previous alpha profile and a 0.51 RGB gain so the stage stays in the dark theme range under the cards. |
...
| `cards/illustrations/face-panel-01.webp` … `face-panel-80.webp` (`.png` masters retained) | Tarot illustration **mat** runtime rasters (center panel), tiered common/uncommon/rare | Local **Z-Image-Turbo** batch (`yarn face-panels:local:zimage`, prompts in `face-panels.zimage.manifest.json`, takes in `face-panels.zimage.picks.json`), installed with `yarn face-panels:install:zimage` (6% edge crop + Lanczos to 520×592), then `yarn face-panels:export-runtime-webp`; URL barrel [`facePanelRasterUrls.ts`](../cardFace/facePanelRasterUrls.ts). Earlier SDXL set: [`batch_local_face_panels.py`](../../../scripts/card-pipeline/batch_local_face_panels.py) | Weighted fallback in [`weightedFacePanelPool.ts`](../cardFace/weightedFacePanelPool.ts); slot order 01-48 common, 49-72 uncommon, 73-80 rare is fixed. Legacy `deck-01..06.svg` stay referenced for `yarn build:card-illustration-manifest`. |
