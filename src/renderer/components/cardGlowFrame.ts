...
export const advanceCardGlowFrame = (
    ...
    const snuff = snuffedAtCandidate == null ? 1 : cardBreakSnuff(time - snuffedAtCandidate);
    ...
    const flare = matchedAt == null ? 0 : cardMatchFlare(time - matchedAt, heat);

    return {
        frame: {
            glowOpacity: Math.min(1.6, levels.runeGlow * breath * snuff + flare),
            spinOpacity: Math.min(1.4, levels.spin * breath * snuff + flare * 0.6),
            spinRotation: reduceMotion ? 0 : -time * levels.spinRate * Math.PI * 2 + phase * 0.2
        },
        memory: { heat, matchedAt, snuffedAt }
    };
};
