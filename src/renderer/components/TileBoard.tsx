...
import TileBoardFeedbackOverlay from './TileBoardFeedbackOverlay';
import TileBoardTrapResolutionFeedback from './TileBoardTrapResolutionFeedback';
...
const TileBoard = forwardRef<TileBoardHandle, TileBoardProps>(function TileBoard(
    ...
    } = useTileBoardLiveRegionRecovery();
    ...
    const sceneReadabilityProps = useMemo(
        () =>
    ...
    const shortLandscapeMobileCamera = cameraViewportMode && width > height && height <= 520;
    const fitMargin = cameraViewportMode
        ? shortLandscapeMobileCamera
            ? SHORT_LANDSCAPE_MOBILE_CAMERA_FIT_MARGIN
            : MOBILE_CAMERA_FIT_MARGIN
        : compact
          ? COMPACT_BOARD_FIT_MARGIN
    ...
    const createDefaultCameraViewport = useCallback(
        (metrics: TileBoardViewportMetrics): TileBoardViewportState => {
            const fittedViewport = createFittedBoardViewport(metrics.fitZoom);
            if (!cameraViewportMode || shortLandscapeMobileCamera) {
                return fittedViewport;
            }

            const mobilePanY = metrics.viewportHeight * 0.11;
            return clampBoardViewport({
                ...metrics,
                ...fittedViewport,
                panY: mobilePanY
        ...
        [cameraViewportMode, shortLandscapeMobileCamera]
    );
    ...
    useEffect(() => {
        ...
        const nextViewport =
            resetRequested || !previousMetrics
                ? createDefaultCameraViewport(nextMetrics)
                : carryBoardViewportForward({
                      nextMetrics,
                      previousMetrics,
                      previousViewport
    ...
    return (
        <div
            ...
            {...{ [REG105_DATA_DAIS]: 'v1' }}
            data-has-dungeon-status-panel={hasDungeonStatusPanel ? 'true' : 'false'}
        ...
        >
            ...
            <TileBoardTrapResolutionFeedback details={trapResolutionDetails} message={trapResolutionMessage} />
            {!baselineWebGl ? (
                ...
                >
                    ...
                    >
                        ...
                        <TileBoardFeedbackOverlay
                            boardChainOpportunityLinesLength={boardChainOpportunity.lines.length}
                            boardChainOpportunitySurfaceView={boardChainOpportunitySurfaceView}
                            boardOpportunityCompassView={boardOpportunityCompassView}
                            boardOpportunityLaneMapView={boardOpportunityLaneMapView}
                            boardStatusChipsView={boardStatusChipsView}
                            compact={mobileCameraMode}
                            focusedPreviewChipProps={{ preview: focusedPreviewChipView }}
                        />
                    </div>
                </div>
            )}
