...
## Problem
**Feature** **flags** **content** **lock** and **release** **readiness** (what ships in demo vs full) — the gate for `REG-070` / `REG-130` RC signoff and `REG-090`–`REG-104` demo matrix.
...
- Connect `REG-115`+ build flavors to `SaveData` and CI; `REG-040` / `REG-030` Steam and `REG-052` (no **online** gate).
