...
- validation diagnostics for stale references, dangling edges, orphan mechanics, missing critical tests, unconsumed graph writes, and active-run `RunState` fields with no production reader.
...
The JSON file is authoritative for tools but not the source of gameplay truth. TypeScript remains authoritative for derived structure, the `RunState` ownership index, and player-visible feedback fields, while `src/shared/gameplay-interaction-graph-data.json` remains authoritative for curated relationships that static analysis cannot infer.
